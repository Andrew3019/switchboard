...
class RolesTest(unittest.TestCase):
    ...
    def test_a_gated_tier_is_refused_at_the_role_that_asked_for_it(self):
        ...
        off and no role gets it until `[routing] gpt_luna_direct_enabled` is true. The ROLE
        list is the mechanical half of the direct-path rule: an agent that splits work,
        routes it or judges somebody else's change may not have this tier, whoever names
        it, while the two implementation leaves may.
        ...
        with self.assertRaises(models.ModelConfigError) as cm:
            roles.get(r, "worker").spec(tier)               # off as shipped
