...
| 9 | ~~Poking a busy agent queues~~ **WRONG — RETRACTED.** The 1→30 count finished instantly, so the agent was already idle when the second prompt landed. **`agent prompt` INTERLEAVES: it is injected into the agent's current turn.** Re-verified with a genuine 60s multi-step turn: poke handled at +13s, the running task did not finish until +63s. | The doorbell is **disruptive** — every `tell` to a working agent interrupts it. `interrupt` differs from `tell` only by sending `esc` first. A real `--when-idle` needs `events.subscribe` (`pane.agent_status_changed`) and therefore a daemon; the same stream answers `sb wait`. **This is the first requirement that genuinely needs one.** |
