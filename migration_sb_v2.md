...
| Object | What it is |
| --- | --- |
| **Task** | The top-level user-facing scope: the boundary you close. An identity, a membership, and a milestone log — not a document (Section 3). |
| **Plan** | One coherent, independently landable workstream inside a Task, covering the full lifecycle of that change. The record of one landable change. |
...
| **Dispatcher** | The long-lived intake agent: it scopes incoming work, creates Tasks, and seeds each with an agent. It is not a report sink. |
...
**The division of labour between the two records is fixed:**

> Plan is the record of one landable change. Task is the boundary you close. Anything a Plan could hold, a Plan holds.
...
* Tasks are created by the human, or by the dispatcher on the human's instruction. A working agent does not create Tasks: delegated work stays inside its own Task as a Plan, a Step, or another agent.
...
### The dispatcher, and what replaced the reporting chain

The dispatcher survives, with one job removed.

**What it keeps** is intake: taking a pile of issues or a loose request, scoping it, combining what belongs together, deciding what becomes one Task and what becomes several, then creating each Task and seeding it with an agent. That is codebase and product reasoning, and it is the job it is good at.

**What it loses** is being the sink every dispatched agent reports back to. Agents it seeds do not report to it. The dispatcher creates the Task and steps out of the chain.

This is the whole of the change, and the two halves are one change:

```text
reply chain removed  →  something else must hold the work  →  the Task holds it
```

Without a container, cutting the relay would leave finished work with nowhere to land and no answer to "is this done, is it stuck, where do I look". The Task is that container, and its membership, derived condition and milestone log are what the chain used to carry.

The dispatcher may hold several Tasks open at once and keep scoping new work into them. It is a long-lived session that accumulates intake context on purpose — it is the thing that knows the backlog.

### Task-level coordination

Where coordination across Plans in one Task is genuinely required — sequencing them, reconciling results — the dispatcher that created the Task is the natural holder, and may act on it without being in the reporting chain.

Responsibility may also be assigned explicitly to an agent in the Task, or moved later by handoff. If nobody holds it and coordination is required, the fallback is the most able **eligible** agent: `live`, and neither `completed` nor `stalled`; among those Switchboard prefers one that owns an `active` Step, falling back to earliest `created_at` (preserved across restore, agent-ID tie-break). If no agent is eligible the human assigns one; a Task where nothing is advancing is already `Stalled` (Section 6) and needs no separate attention item.

This is a fallback so cross-Plan work always has a defined actor. It is not an ownership hierarchy.

### Removed from the current system

| Removed | Replaced by |
| --- | --- |
| The dispatcher as report sink | Task membership, derived state and the milestone log (Section 3) |
...
| Task summary, objective, scope and constraint fields | The milestone log (Section 3) |
...
**Not removed, deliberately:** there is no separate Switchboard-expertise agent. An earlier draft of this design removed the dispatcher and replaced that half of its knowledge with a persistent per-repo advisor session. With the dispatcher kept, the advisor was a patch for a problem that no longer exists. Agents needing Switchboard mechanics use `sb context`, `sb help` and the turn-start guidance triggers (Section 10), or ask the dispatcher. If many agents asking the dispatcher mechanics questions ever makes it a bottleneck, that is the moment to reconsider — after observing it.

---

## 2. Creating and starting Tasks

A Task can be created two ways, and both are cheap.

### Directly

The manual flow behaves almost exactly like spawning a normal Claude Code agent: a prompt field and Create.
...
The other path is the one that handles a pile of work rather than a single request. The human gives the dispatcher issues, links or a loose description; the dispatcher reads them, scopes them, combines what belongs together, and decides what becomes one Task and what becomes several.

For each, it creates the Task and seeds it with one agent and that agent's brief:

```text
issues, links, a loose request
        ↓
dispatcher — scope, combine, decide the split
        ↓
Task A ← seeded agent        Task B ← seeded agent
        ↓                            ↓
   work proceeds                work proceeds
   (no reports back to the dispatcher)
```

The seeded agent owns its Task's work from there. It does not report to the dispatcher, and the dispatcher does not wait on it: Task state surfaces on its own (Section 11), so progress is visible without anyone relaying it.

The dispatcher stays live to scope the next thing. It is not finished when it dispatches — it simply is not in the chain.

**Scoping is the dispatcher's job because it needs codebase and product context.** Deciding that two issues are one change, or that a third is out of scope, is not a Switchboard-mechanics decision. Nothing in Switchboard makes that judgement for it.
...
## 3. Task and Plan state

### What a Task is

A Task is five things, and no more:

```text
identity          id and title
membership        which agents, Plans and worktrees belong to it
lifecycle         open | closed, set by the human
derived condition working | stalled | ready_to_close, computed by Switchboard
milestone log     a short append-only record of what happened
```

There is no Task document. No objective field, no scope, no constraints, no success criteria, no summary to keep current. A Task that has one Plan needs none of them — the Plan already holds the objective, the scope, the verification and the PR, and a second copy at Task altitude is duplicated state that drifts. The only agent obligation in the whole object is appending the occasional line to the log.

### The milestone log

The log answers *what happened here*, in order, in one line each. It is **append-only**: never rewritten, never deleted, never reconciled. A correction is another line.

It is half-written by Switchboard. The mechanical rows cost nobody anything:

| Appended by Switchboard | Appended by an agent |
| --- | --- |
| Task opened and closed | A conclusion reached that no Plan records |
| Plan created, completed | Work deliberately ruled out of scope, and where it went |
| PR opened, merged | A decision the human made in conversation |
| Agent seeded into the Task | Anything else the system cannot see |

A worked example — the agent wrote two of these six lines:

```text
· Task opened — research session-expiry redesign                    [system]
· Settled on refresh-token rotation over sliding expiry             [agent]
· Deprecated API at 2 call sites — out of scope, filed #412 #413    [agent]
· Plan "rotate refresh tokens" created                              [system]
· PR #414 opened                                                    [system]
· PR #414 merged                                                    [system]
```

Rules, because an unbounded log stops being cheap to read:

* One line. A hard cap of roughly 20 words, enforced by the command rather than asked for in prose.
* Appended at genuine milestones only — never per turn, and never as a ritual before `sb done`.
* Never rewritten or deleted.
* The default read is the last handful of lines; `--full` gives the whole log.

**Why append-only rather than a maintained summary.** A summary answers *what is true now* and must be rewritten to stay true, which is where agents over-document and where two agents' edits collide. A log answers *what happened* and cannot be got wrong by appending. For the thing a Task actually needs to carry — orientation for a human returning to it, and cross-Plan context for an agent joining late — chronology is the more useful shape and by far the cheaper obligation.

**Where it stops being merely observational.** Once agents no longer report up a chain, an agent joining a Task's second or third Plan has no other way to learn what the first one concluded. A few lines of log is that. So the log is load-bearing in exactly the multi-Plan case, and pleasant everywhere else — the same object, with no special casing.

### Where information lives

| Store | Content | Written by |
| --- | --- | --- |
| Task | identity, membership, lifecycle, milestone log | Switchboard, plus agent-appended log lines |
| Plan document | Plan summary; editable step *structure* — IDs, names, order, and the kind of a Step being created | Agents |
| System-held Plan state | step *ownership*, step *completion*, step *kind*, the Plan's branch/worktree binding, PR facts | Switchboard |
...
Derived state lives **outside** the editable documents. An agent's write can never set or revert it, and agents must not maintain facts Switchboard can determine itself. In particular, a Step's *completion*, *ownership* and *kind* are system-held, not fields in the agent-submitted Plan document: a whole-document `sb plan edit` carries step names, order, and the kind of *newly added* Steps only, and can never revert `Open PR ✓` or `Merge ✓`, write itself in as a Step's owner (bypassing the `take`/`--assign-step`/`--steal` guards), or change an existing Step's kind. Ownership changes go exclusively through the ownership verbs (Section 4). Judgment-based completion (a Review the Lead Reviewer declares finished) is recorded through a dedicated verb, not by writing a completion flag into the document (Section 4, Section 10).

The **milestone log and the event log are different things.** The event log is complete, mechanical and unread by default — every state transition, for debugging and analytics (Section 12). The milestone log is curated, short, and the thing a person or a joining agent actually reads. The mechanical rows above appear in both; the agent-written lines exist only in the milestone log.

### Editing the Plan document

The Plan document is read and written whole, not mutated field by field:

```text
read compact document → reason about the desired shape → write it back once
```

`sb plan edit` accepts the complete desired document. Not: add step, rename step, assign owner, move step, add another step.
...
* Serialization orders concurrent writes but does not by itself prevent a lost update: an agent that read the document before another agent's write would otherwise clobber it by writing back its whole stale copy. Switchboard prevents this with an implicit base version the agent never reasons about: `sb plan show` returns an opaque version token and `sb plan edit` carries it back automatically. On a stale write Switchboard compares the submitted document to the current one **field by field** against the shared base: fields the submitter did not touch are taken from the current document, and the write is accepted as a merge. Only a genuine conflict — both writers changed the *same* field to different values — is rejected, and then with the current document and a one-line "re-read and re-apply" instruction. An edit carrying no version token is not refused: with no base to merge against it is applied as a plain write of the fields it contains. Agents never construct or compare version tokens by hand, and a true conflict is expected to be rare — this must not become a read-before-write ceremony on every edit.

There is no equivalent for the Task: a Task has no document to edit, only a log to append to.

### Plan summary versus history

The Plan **summary** is a short replaceable field answering *what does another agent need to know about this change now?* It stays compact: not a transcript, not an execution history, not a completion report, not a copy of every finding. Agents tend to over-document, so summary updates are explicitly constrained to the minimum another agent would actually need.

The **history** is the automatic append-only event log. Whenever an agent edits a Plan, Switchboard records the change with enough information to reconstruct before/after state. Agents never maintain a changelog by hand.

### Asking the user

Agents ask the user only when there is a real unresolved decision, never because something is unrecorded.

```text
Bad:  "What should I write in the Task log?"
Good: "Should Google OAuth replace password login or coexist with it?"
```

The line between asking and deciding is a rule, not a feeling, because the costs are lopsided — deciding alone is free, asking the human is the only option that creates a durable object and holds the Task open, so an agent left to judge will drift toward not asking. The rule: **ask the human when resolving it would change scope, constraints, or user-visible behaviour; otherwise decide it yourself and append a log line saying what you decided.** That gives two agents the same answer, and keeps a self-made call visible rather than silently becoming settled fact.
...
**The reminder to do this is delivered, not carried.** It is a turn-start hint (Section 11), not a line in the standing spawn prompt. This is an observed result in the current system, not a preference: the same instruction sitting in the spawn prompt is widely ignored, while the same instruction delivered at turn start, or relayed by another agent as a message, is complied with. Section 11 records why. A Plan-creation norm that lives only in the standing prompt should be assumed not to fire.
...
blocked   `Merge` with its PR open and unmerged, waiting on a human to merge it. Not
          idle, not an attention item on its own (the Plan's `Needs Human Review` item
          is). An owner holding standing authority to merge awaits nothing external, so
          for it `Merge` is `active` and it merges itself. However the merge is done,
          Switchboard observes it and the Step goes to `complete` (Section 4)
...
* **Judgment-completed** Steps (`Review`, `Implement`, `Research`, `Design`) — the accountable owner declares completion with `sb plan complete --step <Step>` (Section 11). Switchboard accepts it only from that owner and rejects it for system-completed Steps.
...
* Each Step carries an opaque **ID** assigned at creation, distinct from its display name. All system-held state — completion, ownership, kind, PR facts, events — is keyed to the ID, never to the name.
...
* Commands name a Step by display name for convenience; Switchboard resolves it to the ID and errors on an ambiguous or unknown name (Section 11).
...
The human is not a Step owner. A human who merges the PR, or tells the owning agent to merge it, acts on the `Merge` Step without owning it ("Changes requested and merge" below).
...
**Review is independent by default**, and an exception is recorded rather than hidden. "Independently reviewed" is a claim a delegator relies on, so Switchboard enforces it instead of taking it on trust: an agent that owns, owned, or is a recorded contributor to an `implement`-kind Step on this Plan cannot `take` or `complete` the `review`-kind Step. The exception is per instance and the agents' own call — not a repo setting and not a human grant: an agent that decides a self-review is right for this change passes an explicit self-review override on that `take` or `complete` (or, where a change record's review is derived done as its PR opens, on that `open-pr` or `comment`), and the Step is then recorded as `review_independence: self-reviewed` and surfaced on the PR comment. A recorded contributor is an owner on the Step's record — its current owner and every agent its take, steal, release, complete and tick events name — so this needs no commit-attribution heuristic (agents on one Plan share a worktree and one git identity, so git authorship could not distinguish them anyway).
...
A review is a **message** to the implementing agent — what to fix, or that it is clear — not a durable object, and nothing gates `Merge` on it. Nits are dropped; minors may be fixed in place by the reviewer and named; anything needing code changes returns to the implementing agent. This is also the reviewer's reopen path: a Lead Reviewer whose review requires code changes reopens `Implement` (the same requested-changes loop the user path uses), rather than the loop being reachable only when *the user* asks.
...
PR observed merged                     → Merge complete
```

`Open PR` is a bundled Step, so it completes when the whole operation succeeds, not merely because a PR exists. Observing a PR on the remote attaches the PR reference; it does not by itself imply the local checks and PR-comment portions succeeded. The "required checks" it runs are the repo-configured list of **local** commands (Section 10), defaulting to the repo's test command; a repo may add lint, build or type checks. These run before completion; if one fails, or the PR opens but the summary comment does not post, the Step enters `failed`, carrying which sub-operation failed, and its owner retries with `sb plan step retry`. The operation is idempotent: retrying reuses the existing PR rather than opening a second one.

**Remote CI is not part of `Open PR`, and not a Switchboard merge precondition.** `Open PR` does not wait on the remote's checks, CI is not a Step of its own, and Switchboard gates nothing on it: a repo's required status checks are the host's own gate, which refuses the merge itself while they are red. Nor does Switchboard gate `Merge` on a review or an authorization — landing is whoever is authorized merging after a look (below).
...
```text
Implement → Review → Open PR → Merge (by the human, or an agent under standing authority) → Plan complete
```
...
Once a PR is opened and its merge is a human's, Switchboard derives that the Plan is waiting on human review and surfaces it. No agent turn is spent declaring "now waiting for Andrew".

```text
Open PR ✓   Merge ○   → Needs Human Review   (unless the owner merges under standing authority)
```

The worker that owns the change end-to-end owns the incomplete `Merge` Step while the PR waits. Unless it holds standing authority to merge, that Step is `blocked` (waiting on a human's merge, an external event Switchboard observes), so the worker is **not** derived as stalled: its derived state is `awaiting external`, which is excluded from stalled-agent attention because the `Needs Human Review` item on the Plan already represents the same wait. For the same reason `sb done` is not blocked by a `blocked` Step (Section 6) — the owner may stay live-and-idle, or `sb done` and be reactivated. There is no approval to record and no gate to clear: the human merges from the browser or GitHub, or tells the owner to merge it — a message, which like any `sb tell` wakes a `completed`-but-live owner and is held for a not-live one until restore. An owner with standing authority to merge waits on nobody: its `Merge` is `active` and it merges itself. However the merge happens, Switchboard observes it (Section 9, remote-fact observation) and completes the Step. Either way the wait for a human surfaces only as `Needs Human Review` — never a phantom stall or a phantom unowned Step.
...
Reopening a Step **suspends every later Step to `pending`** (a defined `complete`/`blocked`/`active → pending` transition used for this cause only), because eligibility depends on predecessors being complete and they no longer are. A suspended `Merge` is not eligible, so the Plan leaves `Needs Human Review` while the re-review is open. System-completed Steps keep their established facts while suspended (the PR stays open); once eligibility returns, the suspended `Open PR` Step goes from `pending` back to `active` and its owner is woken to re-run the bundle against the new head with `sb plan open-pr` — refreshing the PR summary comment and re-running the local required checks, and reusing the existing PR rather than opening a second one — which is what keeps the PR-facing information current after the fixes land. When the reopened Step re-completes, the suspended Steps re-eligibilize in order. Throughout, the same worker fixes, re-reviews, and pushes the updated PR; the Plan log and PR-facing information stay current.

**Landing is a merge, not a gate.** There is no authorization object, no approval or pre-approval to record, and no merge precondition for Switchboard to check. Whoever is authorized to merge does so after a look: the human clicks merge, or an agent merges under standing authority it has been given. An agent without that authority does not merge: `Merge` is `blocked` and the Plan waits as `Needs Human Review`. An owner holding it has nothing to wait for: `Merge` is `active` and the owner merges it itself. However the merge happens — the browser, GitHub, or `sb plan merge` — Switchboard observes it and completes the `Merge` Step (Section 7).

`sb plan merge` is the agent's way to do the merge, not a gate in front of it. It records on whose authority the change landed — `human` (a person named the head to land) or `standing` (agent-decided) — and makes one safety check at execution time: it re-fetches the live PR head (Section 7) and refuses unless that is the head the recorded PR, verification and review cover, so a push since the last look cannot slip an unreviewed head through. It re-runs no checks and reads no CI status.

After requested changes the user may want to look at the updated result again before merging it, or may tell the agent up front to merge once the fixes land.
...
`pending` Steps (not yet eligible) are not work the agent can act on, so they do not defeat `awaiting external`. Only an `active` Step counts as actionable owned work. A `Merge` Step is `blocked` (not `active`) while its open PR waits on a human's merge, so its owner is `awaiting external`, never `stalled`, and the `Needs Human Review` item already represents that wait. An owner with standing authority to merge has nothing to wait for: its `Merge` is `active`, and it is not excused from stall derivation. Nothing else holds `Merge`: there are no merge preconditions for an agent to clear (Section 4).
...
`sb ask <target>` creates a durable Question associated with the asking agent and its Task. The target is one of `human`, `parent`, or an explicit `<agent-name>` (a Task's seeded agent has no `parent`, so it targets `human` or a named agent — including the dispatcher that created the Task, which is a normal addressable agent, not a reporting line). A `human`-targeted Question surfaces in the browser attention queue; an agent-targeted one is visible in the Task view but does not enter `Needs You`, since no human action is useful on it. Creating one does not by itself mean the agent is blocked.
...
| Switchboard mechanics or configuration | `sb context`, `sb help`, the turn-start guidance triggers; the dispatcher if none of those answer it |
...
* **Orphaned Question.** A Question whose asker will not come back to process it is auto-cleared after `attention_timeout` (Section 10), with an event recorded, so it cannot permanently block progress. Three cases: an asker that is neither live nor restorable (its Task was closed, or it was cleaned up) has its Question auto-`withdrawn`; an asker that is `completed`-but-live and has left a Question `pending` or `answered` unprocessed past the timeout has it auto-`resolved` (the answer, if any, is preserved in the event log); and an agent-targeted Question whose *target* becomes not restorable — the answer can never arrive — is auto-`withdrawn`, which also releases the asker's `waiting on agent`. Without this, an agent-targeted Question answered after its asker had already gone `done` — a reviewer answering a worker that has finished — would keep the Task out of `Ready to Close` forever, unsurfaced (agent-targeted Questions are not in `Needs You`). This timer is separate from `stall_threshold` so tuning stall detection does not change how long an orphaned Question survives.
...
There is no rigid `one Task = one worktree` or `one Plan = one worktree` rule. A Task may contain several worktrees; a Plan may use the Task's worktree or another one. A Plan nonetheless has one **primary branch and worktree**, recorded as system-held state (Section 3) at `sb plan create` or when its first commit is observed. That binding is what the branch-dependent rules read: the completion guard tests whether it carries commits, and `Open PR` asserts the PR head is this branch. Review independence does not read it — it is tested against the Step ownership record, not git authorship (Section 4). Auxiliary worktrees an agent creates inside a Plan are folded back into that primary branch before the PR opens.
...
Role, model, presets and other starting configuration resolve at spawn. Presets are the exception that may also be invoked later, loading only when needed. Changing repo configuration affects new agents and never silently mutates running sessions.
...
## 11. Agent context and the command surface
...
1. **Minimal universal context.** Every agent knows only: that it belongs to a Switchboard Task; its Task identity; its current assignment; that it can delegate and spawn; how to message relevant agents; how to ask the human or its parent; how to report `done`; how to inspect its own context; how to discover further capabilities. Detailed spawn and worktree options load only when needed.
...
No speculative hints such as "seems like it might need review" or "has been coding for a while". Where the moment cannot be identified deterministically, rely on `sb context` and `sb help`.
...
sb plan create | show | take <step> | release <step> | complete --step <step>
              | reopen --step <step> | open-pr | merge | step retry <step> | edit
sb task show | log "<one line>" | close
...
| `merge` | merges the PR on a human's or the agent's standing authority, recording which; re-fetches the live head and refuses unless it is the reviewed head — no other gate (Section 4) |
...
| `sb task log "<line>"` | appends one milestone line to the Task, capped at roughly 20 words (Section 3) |

`open-pr` is a Switchboard-owned operation — that is what makes it idempotent and retryable — so an agent completes `Open PR` by calling it, never by running `gh` itself. `Merge` completes when Switchboard observes the merge, so a human merging in the browser or on GitHub completes it just as `sb plan merge` does; the verb is how an agent merges with the head check applied. Each verb is accepted only from the owner of the corresponding Step (or an agent that first `take`s it), so the accountable owner the wake and stopped-owner backstop target is also who acts.
...
Further high-level operations may be added if they represent genuine recurring lifecycle actions. Underlying database operations may remain granular internally; agents interact with the higher-level transaction. Plan editing semantics — whole documents, agent-writable fields only, serialization handled by Switchboard — are defined in Section 3. A Task has no document to edit; `sb task log` appends, and nothing rewrites.
...
| `sb ask` | Routes structured requests for information; every ask creates a durable Question with lifecycle and attention semantics |
...
Items are derived from current authoritative state and disappear automatically when the condition resolves — question answered, the PR merges, stalled agent resumes, failed Step retried, worktree reclaimed. No manual dismissal. When one condition would raise two items — a `stalled` agent that also owns an `active`/`failed` Step raises both a stalled-agent item and a stopped-owner Step item — the Step item is shown and subsumes the bare agent item, since it names the actionable work; they are not counted twice. It must not become a general notification feed, and `Ready to Close` Tasks belong in the Tasks view, not here.
...
New Task, hand work to the dispatcher, open/focus an agent terminal, answer a Question, merge a PR (or merge it on GitHub), request changes (reopen), retry a failed Step, assign a Task coordinator, close a Task, restore the environment, edit repo configuration, and spawning an agent as a convenience.

Humans generally do not edit Plans in the browser. Plans are agent-managed execution structures, and human interaction with them is decisions and merges rather than low-level maintenance. Inspecting Plans should nonetheless be easy.

Browser actions and CLI/agent actions operate through the same underlying state and operations. There are no browser-only orchestration semantics: merging from the browser and telling an agent "looks good, merge it" enter through different interfaces but update the same Task/Plan/message state.
...
## 13. Logging, history, and analytics
...
This is the single authoritative list of the states this document relies on: their entry and exit triggers, who sets each, and whether it raises a `Needs You` attention item. Where Section 12 leaves "task status vocabulary and precedence, card layouts and queue organization" open to iteration, this is the fixed set it iterates *beyond* — the derivation engine, `sb context`, and the browser must all agree on these.
...
An orphaned Question — asker not restorable, or `completed`-but-live and unprocessed past the timeout — is auto-`withdrawn` or auto-`resolved` respectively (Section 7).
...
| `blocked` | a `Merge` Step, eligible, its PR open and waiting on a human's merge (an owner with standing authority to merge has `Merge` `active` instead) | merge observed → `complete`; earlier Step reopened → `pending` | derived | no (the Plan's `Needs Human Review` item covers it) |
...
`Needs Human Review` spans the PR-open-to-merged interval of a Plan whose merge is a human's, and it is what the `Needs You` queue heads as "PRs ready for review"; a Plan whose owner merges under standing authority stays `in progress` until the merge is observed, so a Plan is never without a state. There is no `awaiting merge` or authorization sub-state: the merge itself is what moves the Plan to `complete` (Section 4). A Plan is never `complete` with zero Steps — the vacuous case is `in progress` (Section 4).
...
| --- | --- | --- |
| `open` | created by the human, or by the dispatcher on the human's instruction | closed (human action) |
| `closed` | human closes it | terminal |

A Task also carries a **milestone log** (Section 3): append-only, one line each, written partly by Switchboard and partly by agents. It is not a state and never gates anything.
