...
If a coordination need is open and no eligible agent can hold it, Switchboard surfaces `Task: coordination needed — no eligible agent` as an attention item so the human assigns one, rather than letting the work silently strand.
...
**Review must be independent, and Switchboard enforces it** — this is what makes "independently reviewed" a fact a delegator can trust rather than a phrase an agent can type. The check is on authorship of the change *as it stood when Review began*: Switchboard rejects taking, assigning or completing the `review`-kind Step for any agent that owns or owned an `implement`-kind Step on this Plan, or that authored commits on the Plan's branch (Section 8) before it took `Review`. Fixes the reviewer itself commits *after* taking `Review` (the minor in-place fixes below) do not count against it, so completing its own Review is never deadlocked by its own repair. The reviewer is therefore always a different session from the author. (A repo that genuinely wants to allow self-review overrides this in configuration, and a self-reviewed completion is then recorded as `review_independence: self-reviewed` and surfaced on the PR comment, so the exception is visible rather than silent.)
