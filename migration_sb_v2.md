...
**Review must be independent, and Switchboard enforces it** — this is what makes "independently reviewed" a fact a delegator can trust rather than a phrase an agent can type. Switchboard rejects `sb plan take Review`, `--assign-step Review` and `sb plan complete --step Review` for any agent that owns (or owned) `Implement` on this Plan or has commits on the Plan's branch head. The reviewer is therefore always a different session from the author. (A repo that genuinely wants to allow self-review overrides this in configuration, and a self-reviewed completion is then recorded as `review_independence: self-reviewed` and surfaced on the PR comment, so the exception is visible rather than silent.)
...
A review produces durable **Findings**, not just a chat message — because "no unresolved major review finding" is a merge precondition (below) and a precondition needs something real to test. A Finding is a small durable object `{plan, step, severity (major|minor|nit), raised_by, resolved_by, resolved_at}`, created with `sb plan finding raise` and cleared with `sb plan finding resolve`. Nits are dropped rather than recorded; minors may be fixed in place by the reviewer and named; a major returns to the implementing agent. An unresolved `major` Finding blocks `Merge` and, if it sits unresolved past `failed_step_timeout`, becomes a `Needs You` item. This is also the reviewer's reopen path: a Lead Reviewer that raises a major Finding requiring code changes reopens `Implement` (the same requested-changes loop the user path uses), rather than the loop being reachable only when *the user* asks.

`reopen` is permitted to the Step's owner, to the agent that owns the Plan end-to-end, to a Lead Reviewer acting on its own major Finding, and from a human action; any other agent raises a Finding or asks instead of reopening. This bounds a verb whose blast radius is large (it suspends every later Step, Section 4 above).
...
Switchboard gates `done` on more than Step ownership, because assignment scope — not Step ownership — is what `done` actually means, and the two differ (a Worker assigned end-to-end is not done at Implement). So assignment scope is made machine-visible: `sb spawn --own-plan <plan>` records that the agent owns a Plan end-to-end, and `done` is then blocked until that Plan is complete or its ownership has been handed off, exactly as the prose requires. This closes the two one-command false-completion exits: an agent cannot `sb spawn` a child with its own assignment and immediately `sb done` (its end-to-end ownership is unmet), and it cannot escape work by releasing a Step (below).
...
| `waiting on child` | idle awaiting a report from a `live`/`restorable` delegated-subtask child of this agent's assignment, that child not itself idle | child reports, is reassigned, becomes not restorable, or is itself idle past `stall_threshold` | derived | no |
...
| `failed` | a bundled sub-operation or local check failed; or a system Step whose remote fact was later observed false (PR closed) | `step retry` succeeds → `complete`; retry fails → stays `failed` | Switchboard | **yes** immediately if its owner has stopped, else past `failed_step_timeout`; a `failed` owned Step also blocks the owner's `sb done` |
