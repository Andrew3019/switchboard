...
The fallback coordinator is computed **only when a coordination need actually exists** — Plans awaiting sequencing, or another cross-Plan decision — not continuously, so no agent is handed a coordinator hat while nothing needs coordinating and the role does not churn on unrelated status changes. When a need exists, Switchboard picks the most able **eligible** agent: eligible means `live` and neither `completed` nor `stalled`; among those it prefers one that owns an `active` Step in the Task (an agent actually driving work), falling back to earliest `created_at` (preserved across restore, agent-ID tie-break) only if none does — so the hat lands on a working agent rather than systematically on a parked, handed-off researcher. The holder is injected a hint that **carries the work**, not a bare title: "you now hold Task-level coordination — Plans A, B complete; Plan C awaiting sequencing", with the `sb task show` pointer. If the holder later becomes ineligible while a need is still open, coordination is handed off with the same work-carrying hint rather than a bare "you no longer hold", so in-flight coordination is never dropped silently.

If a coordination need exists and no eligible agent can hold it, Switchboard surfaces `Task: Plans awaiting sequencing — no coordinator` as an attention item so the human assigns one, rather than letting the work silently strand.
...
| System-held Plan state | step *ownership*, step *completion*, step *kind*, PR/CI facts | Switchboard |
...
* Because the scribe writes the very scope it is later measured against, *narrowing* it is surfaced, not silent: an edit that **removes** an item from `scope.in`, `constraints`, or `success criteria` raises a `Needs You` confirm item naming the removed item and the agent (additions and clarifications are free). This is the same visibility the agent-relayed approval gets (Section 4) — an agent quietly shrinking what the human asked for is exactly as consequential as an agent quietly authorizing a merge.
...
**Review must be independent, and Switchboard enforces it** — this is what makes "independently reviewed" a fact a delegator can trust rather than a phrase an agent can type. The check is on authorship of the change *as it stood when Review began*: Switchboard rejects `sb plan take Review`, `--assign-step Review` and `sb plan complete --step Review` for any agent that owns or owned `Implement` on this Plan, or that authored commits on the branch before it took `Review`. Fixes the reviewer itself commits *after* taking `Review` (the minor in-place fixes below) do not count against it, so completing its own Review is never deadlocked by its own repair. The reviewer is therefore always a different session from the author. (A repo that genuinely wants to allow self-review overrides this in configuration, and a self-reviewed completion is then recorded as `review_independence: self-reviewed` and surfaced on the PR comment, so the exception is visible rather than silent.)
...
`pending` Steps (not yet eligible) are not work the agent can act on, so they do not defeat `awaiting external`. Only an `active` Step the agent can actually progress counts as actionable owned work. In particular, an agent whose only remaining Step is a `Merge` that is `active` and authorized but *barred* by a Plan-level precondition it cannot itself clear right now — red CI, or an unresolved `major` Finding it does not own — is `awaiting external`, not `stalled`: the corresponding attention item (PR checks failed, or the open major) already represents the block, and the owner is woken to merge when the precondition clears. (A red CI the worker *can* fix is different — that is work, and the worker does it.)
...
If none of those explains it and the agent has done nothing for longer than `stall_threshold` (a repo-configurable default, Section 11), it is `stalled` — the single derived state for unexplained inactivity. An agent that has never been given an assignment (the blank agent of a no-prompt Task, Section 2) is exempt: with nothing assigned, its idleness is expected, not a stall, and its Task reaches `Ready to Close` once that agent has done at least one turn's work and stopped. "Activity" that resets the clock is a turn end, any `sb` command, or any runtime tool call Herdr exposes; a session that died and one thinking for a few minutes are told apart by the threshold. There is one agent-level stalled state, not a graded "suspicious / potentially / confirmed" ladder. `stall_threshold` does not run against an agent whose Task is `Ready to Close`: its inactivity is fully explained by there being no work left.

**A stopped owner does not hide an incomplete Step.** An owned incomplete Step that is not `blocked` (an `active` or `failed` Step) is normally covered by its owner being at work; but a Step becoming actionable should first try to *wake* its owner, and only surface to a human if that fails. So on any transition that makes an owned Step actionable — `pending → active` as predecessors complete, `blocked → active` when approval lands, a Step entering `failed` — Switchboard delivers a message to the owner, reactivating a `completed`-but-live owner and holding for a not-live one until restore. This keeps the ordinary Step-to-Step handoff inside the agent tree rather than poisoning `Needs You` with routine items. Only when the wake cannot land (owner not restorable) or is not acted on within `owner_wake_timeout` does the Step surface as a `Needs You` item (Section 13). (A `blocked` Step is never released — Section 6 above — so there is no unowned `blocked` Step to surface.)
...
Targeted mechanical checks remain where they are high-confidence, such as blocking `done` while the agent owns an incomplete non-`blocked` Step (`active` or `failed`) or an end-to-end Plan that has not landed (Section 6).
...
Agent assigned end-to-end Plan ownership (--own-plan)
...
* a Task with Plans awaiting sequencing and no eligible coordinator (Section 1)
...
| `Stalled` | unfinished work and no agent advancing it: a `stalled` agent, or every agent idle with no explanation, or no live agent present at all | the condition resolves | derived | **yes** |
