...
Because Switchboard holds no cross-Plan dependency graph (Section 4), a coordination need is not something it can infer — it is **declared as a durable record**: an agent (or the human) raises one with `sb task need-coordination raise "<what needs sequencing or deciding>"`, and that record, not a guess about Plan state, is the derivable trigger; a coordinator clears it with `sb task need-coordination resolve`. While such a record is open, Switchboard computes the fallback coordinator (it does not otherwise), picking the most able **eligible** agent: eligible means `live` and neither `completed` nor `stalled`; among those it prefers one that owns an `active` Step in the Task (an agent actually driving work), falling back to earliest `created_at` (preserved across restore, agent-ID tie-break) only if none does — so the hat lands on a working agent rather than systematically on a parked, handed-off researcher. The holder is injected a hint that **carries the work**: the text of the open need plus the `sb task show` pointer. If the holder later becomes ineligible while the need is open, coordination is handed off with the same work-carrying hint rather than a bare "you no longer hold". The record clears when a coordinator resolves it or the human closes it.

If a coordination need is open and no eligible agent can hold it, Switchboard surfaces `Task: coordination needed — no eligible agent` as an attention item so the human assigns one, rather than letting the work silently strand — but not while the Task still has `restorable` agents merely awaiting restore, so a machine restart does not flash this item on every open Task before `sb restore` runs (the same restart guard Sections 6 and 9 apply to stalls).
...
| Plan document | Plan summary, step *structure* (names and order only) | Agents |
...
The human remains the authority; agents record the resulting structured state. The one direct-write exception is the human reverting an agent's scope narrowing from the browser (the confirm/revert action on that item, below), which restores the removed `scope.in`/constraint/success-criterion — the human correcting the scribe, consistent with being the authority over scope. The writer is otherwise whichever agent currently holds the relevant understanding — a researcher shaping the problem, a worker discovering a cross-cutting constraint, the agent that received a user decision, or the agent holding Task-level responsibility.
...
blocked   `Merge` awaiting a tracked merge precondition Switchboard follows — approval, CI
          not `red`, or an unresolved `major` Finding — not idle, not an attention item on
          its own. When the last precondition clears the Step returns to `active` for the
          owner to merge; if a human merges directly, Switchboard observes it and the Step
...
**Steps have a stable identity and a fixed kind.** Each Step carries an opaque ID assigned at creation, distinct from its display name, and a **kind** — `implement`, `review`, `research`, `design`, `open_pr`, `merge`, or a repo-defined kind — set by Switchboard at creation from the declared step type, immutable thereafter, and independent of the display name. Kind, not name, decides whether a Step is system-completed (`open_pr`, `merge`) or judgment-completed, and whether review-independence applies (`review`). So renaming `Open PR` to `Raise PR` cannot turn a system Step into one the agent completes itself, and naming a Step `Ship it` cannot smuggle in a merge the agent declares done: an unrecognized display name is just a judgment Step with no special powers, and a change that puts commits on a branch cannot reach Plan `complete` without a completed `merge`-kind Step (otherwise the Plan stays `in progress` and surfaces). All system-held state — completion, ownership, kind, PR/CI facts, events — is keyed to the ID, never to the name. This is what makes "an agent's whole-document write can never revert completion" actually hold: a document that renames a Step keeps the same ID, kind and completion; a Step submitted with no ID is a new Step; a previously-present ID that is absent is a deletion. Because eligibility depends on order, and order is agent-writable, Switchboard **rejects** a structural edit that would reorder or delete a `complete` system Step, or move an incomplete Step ahead of a `complete` one — the edit is refused with the reason, rather than silently changing what has already landed. It also validates kind arrangement at create and edit, refusing (with a reason) more than one `open_pr`-kind or `merge`-kind Step, or an `open_pr`/`merge` pair out of order or half-present — the two kinds appear together and in order (`merge` after `open_pr`) or not at all — so a mis-ordered Plan like `Implement, Merge, Open PR`, or a `Merge` with nothing to merge, is caught at authoring time rather than stranding silently later. An `open_pr`-kind Step also requires a `review`-kind Step before it, so a change that opens a PR always carries the independent review the landing model depends on rather than silently omitting it. (`sb plan complete --step`, `reopen`, `take` and the rest name a Step by its display name for convenience; Switchboard resolves the name to its ID and errors on an ambiguous or unknown name — Section 12.)
...
The ownership operations (`take`, `release`, `complete`, `reopen`, `--steal`) are serialized through Switchboard exactly as document edits are, so the "exactly one accountable owner" invariant is mechanized, not merely asserted: two agents racing `take` on the same `unowned` Step resolve to one winner and one refusal, and `complete` racing a `--steal` resolves deterministically by arrival order. `--steal` is restricted to a Step whose current owner has stopped (`completed`, `stalled`, or not restorable) — taking a Step from a live, working owner is a coordination failure that belongs in a Question or a human item, not a verb — and it delivers a `NORMAL` message to the previous owner naming the Step and the new owner, so a steal is never silent (mirroring the symmetric notification the Task-coordinator handoff uses).
...
**Review must be independent, and Switchboard enforces it** — this is what makes "independently reviewed" a fact a delegator can trust rather than a phrase an agent can type. The check is on authorship of the change *as it stood when Review began*, tested against system-held state rather than git identity (agents on one Plan share a worktree and commit under one git identity, so git authorship cannot distinguish them): Switchboard rejects taking, assigning or completing the `review`-kind Step for any agent that owns, owned, or is recorded as a contributor to an `implement`-kind Step on this Plan. Step contributorship is already tracked (an `Implement` Step may have several contributing agents), so this needs no fragile commit-attribution heuristic. Fixes the reviewer itself commits *after* taking `Review` (the minor in-place fixes below) do not count against it, so completing its own Review is never deadlocked by its own repair. The reviewer is therefore always a different session from the author. (A repo that genuinely wants to allow self-review overrides this in configuration, and a self-reviewed completion is then recorded as `review_independence: self-reviewed` and surfaced on the PR comment, so the exception is visible rather than silent.)
...
A review produces durable **Findings**, not just a chat message — because "no unresolved major review finding" is a merge precondition (below) and a precondition needs something real to test. A Finding is a small durable object `{id, plan, step, severity (major|minor|nit), title, body, raised_by, resolved_by, resolved_at, resolution_note}`, created with `sb plan finding raise` and cleared with `sb plan finding resolve <id>`. The `title`/`body` are what the implementing agent and the `Needs You` item render; `resolution_note` records how it was addressed. Resolving a `major` Finding is restricted to the agent that raised it or the Review-step owner, so the implementing agent a major blocks cannot simply clear its own gate — the same bounding every other gate-affecting verb has. Nits are dropped rather than recorded; minors may be fixed in place by the reviewer and named; a major returns to the implementing agent. An unresolved `major` Finding blocks `Merge` and, if it sits unresolved past `failed_step_timeout`, becomes a `Needs You` item. This is also the reviewer's reopen path: a Lead Reviewer that raises a major Finding requiring code changes reopens `Implement` (the same requested-changes loop the user path uses), rather than the loop being reachable only when *the user* asks.

`reopen` is permitted to the Step's owner, to the agent that owns the Plan end-to-end, to a Lead Reviewer acting on its own major Finding, and from a human action; any other agent raises a Finding or asks instead of reopening. This bounds a verb whose blast radius is large (it suspends every later Step, Section 4 above). `reopen` is rejected once the Plan's `merge`-kind Step is complete: a merged Plan is terminal, and a post-merge concern is a new Plan, not a reopening of a landed one.
...
Both entry paths write this same object, but they differ in how far they are trusted. A browser `Approve` is the human acting in a Switchboard-visible way and resolves the `Needs Human Review` item outright. The agent-driven path — the user tells the agent "looks good, merge it" — is equally valid but is relayed, so it carries a risk the agent misread a "looks good" that was about a design or a summary rather than this diff. The agent records it with `sb plan approve`, quoting the user's words into the event log. The relayed path may record only an `approval` (head-bound, the diff the human just looked at) — never a `pre-approval`: the one kind that authorizes content the human has not seen requires the browser, where the human picks it explicitly, so an agent cannot upgrade a casual "looks good" into standing merge authority. A relayed `approval` is marked `granted_by: agent-relayed` and `confirmed: false`. It is a live authorization, so it lets the merge proceed (the fast path is not blocked when the relay was correct) and moves the Plan to `awaiting merge` — but it raises its own durable `Needs You` item, `Merge (this diff) authorized by <agent> from your message — confirm or flag`, derived from the authorization being `agent-relayed`, not `confirmed`, and not `revoked`. That item is **independent of the merge**: it survives it, becoming a post-hoc `Merged on a relayed approval — confirm or flag` if the merge lands first, so the human's chance to notice a misread is never destroyed by the merge racing ahead. It clears when the human confirms (`confirmed: true`) or flags/revokes it (`revoked`), which are browser actions (Section 13); a `revoke` after the merge does not un-merge — it records the misread and surfaces it for the human to unwind deliberately. It also clears if the approval is invalidated by a later push before any merge (the head it named was superseded and never used), with an event recorded, so a fix cycle that pushes over an unconfirmed relayed approval does not leave a stale item behind or accrete one per round. Whether review is needed at all is the human's call, expressed by which kind they grant; absent any, an agent does not merge.
...
`pending` Steps (not yet eligible) are not work the agent can act on, so they do not defeat `awaiting external`. Only an `active` Step counts as actionable owned work. A `Merge` Step is `blocked` (not `active`) whenever *any* of its tracked merge preconditions is unmet — awaiting approval, CI `red`, or an unresolved `major` Finding — so its owner is `awaiting external`, never `stalled`, by the definition above with no special case: the corresponding attention item (needs review, PR checks failed, or the open major) already represents the block, and the owner is woken to merge when the last precondition clears. There is no can-fix-vs-cannot-fix judgment for Switchboard to make: a worker that decides to fix red CI reopens `Implement` (code fix) or its push simply resets things, and is `active` again either way.
...
**A stopped owner does not hide an incomplete Step.** An owned incomplete Step that is not `blocked` (an `active` or `failed` Step) is normally covered by its owner being at work; the backstop covers the case where the owner has *stopped* (`completed`, `stalled`, or not restorable) while the Step is still open. It works in two beats, both evaluated on the derivation tick rather than only at a transition — so an owner that goes `stalled` while already holding an `active` Step is caught just as an owner that stopped earlier is. First, *wake*: whenever such a Step's owner is stopped-but-restorable and no wake has yet landed, Switchboard delivers a message to the owner (reactivating a `completed`-but-live owner, held for a not-live one until restore), which keeps the ordinary Step-to-Step handoff inside the agent tree rather than poisoning `Needs You`. Then, *surface*: only when the owner is not restorable, or the wake has not been acted on within `owner_wake_timeout`, does the Step surface as a `Needs You` item (Section 13). (A `blocked` Step is excluded from this backstop because its owner is by definition reactivatable and the PR item already represents the wait; it also can never be released, so it never becomes unowned.)
...
* **Orphaned Question.** A Question whose asker will not come back to process it is auto-cleared after `orphan_question_timeout` (Section 11), with an event recorded, so it cannot permanently block progress. Three cases: an asker that is neither live nor restorable (its Task was closed, or it was cleaned up) has its Question auto-`withdrawn`; an asker that is `completed`-but-live and has left a Question `pending` or `answered` unprocessed past the timeout has it auto-`resolved` (the answer, if any, is preserved in the event log); and an agent-targeted Question whose *target* becomes not restorable — the answer can never arrive — is auto-`withdrawn`, which also releases the asker's `waiting on agent`. Without this, an agent-targeted Question answered after its asker had already gone `done` — a reviewer answering a worker that has finished — would keep the Task out of `Ready to Close` forever, unsurfaced (agent-targeted Questions are not in `Needs You`). This timer is separate from `stall_threshold` so tuning stall detection does not change how long an orphaned Question survives.
...
If the agent takes a **user-initiated** turn (one started by the human typing into the session, which the runtime can distinguish from a delivered message or the agent's own continuation without reading the text) after the pending-question metadata was injected, but neither resolves nor withdraws the Question, Switchboard marks it `possibly answered out of band` and drops it from `Needs You` while keeping it visible in the Task view. The signal required is only "the human just interacted here", never the content of what they typed — so this respects the rule that correctness must not depend on intercepting terminal input, while still not firing merely because the agent kept working on something adjacent. The queue errs toward silence, the Task view toward completeness. Crucially the drop is not permanent: at `orphan_question_timeout`, if the asking agent is still live and has not gone `completed`, the still-unresolved Question returns to `Needs You` so a real question cannot vanish forever on a coincidence; if instead the asker has since gone `completed` (it will not return to process it), the orphaned-Question rule below auto-resolves it. The asker's state at the timeout decides which applies, so the two rules never both fire. It also still counts as unresolved for `Ready to Close`, so the work is not treated as finished until the agent actually resolves it.
...
* `owner_wake_timeout` — how long after the wake is delivered to a stopped owner, without being acted on, before the Step surfaces as an attention item; a wake held for a not-live owner counts as not acted on (Section 6).
* `orphan_question_timeout` — the general "long enough to be sure" Question interval, covering both its uses in Section 7: auto-clearing a Question whose asker will not return, and returning a `possibly answered out of band` Question to the queue.
* `failed_step_timeout` — how long a gate-blocking condition goes un-recovered before it raises an attention item: an un-retried `failed` Step, or an unresolved `major` Finding (Section 4 / Appendix A).
...
* `review.allow_self_review` — the off-by-default override that relaxes review independence (Section 4).
...
Agent becomes Task coordinator on a real coordination need
                                       → inject the work-carrying coordination hint once
...
sb task show | edit | need-coordination raise|resolve
sb ask <target> | answer <qid> | resolve | withdraw | escalate
```

`sb plan take <step>` takes a Step for the calling agent; `release <step>` gives up ownership, leaving the Step `unowned`; `complete --step <step>` records judgment-based completion by that Step's owner (rejected for system-completed Steps); `reopen --step <step>` moves a `complete` judgment Step back to `active` (for a requested-changes loop; rejected for system-completed Steps); `open-pr` runs the `Open PR` bundle (checks, PR, summary comment) and `merge` executes the `Merge` — both are Switchboard-owned operations, which is what lets `open-pr` be idempotent (reuse the live PR) and `merge` re-fetch the head and re-check the four merge preconditions at execution time (Section 4); an agent completes these Steps by calling these verbs, never by running `gh` itself. Each is accepted only from the owner of the corresponding Step (or an agent that first `take`s it), so the accountable owner the wake and stopped-owner backstop target is also who acts. `step retry <step>` re-runs whichever of those bundles is `failed`, idempotently; `finding raise|resolve` creates and clears the durable review Findings that gate `Merge`, and `finding resolve` on a `major` is restricted to the raiser or the Review owner (Section 4); `approve` records a human approval (relayed `approve` records only a head-bound approval, never a pre-approval — Section 4). `sb ask answer <qid>` is how an agent answers a Question targeted at it. End-to-end Plan ownership (which the `done` gate holds until the Plan lands, Section 6) is recorded on the agent that runs `sb plan create` by default, with `--no-own` to opt out. It is assigned to a new agent with `sb spawn --own-plan <plan>`, and handed to an already-running agent with `sb tell --handoff --own-plan <plan>` — the same handoff-to-an-existing-agent path handoffs use (Section 6), which is one of the two ways the `done` gate is satisfied (land the Plan, or hand ownership off). A Plan's initial structure is provided in one operation (`--title "OAuth migration" --steps "Research,Design,Implement,Review,Open PR,Merge"`). Each step may state its kind as `Display Name:kind` (`"Land it:merge"`); a bare name resolves to the built-in kind of a recognized default step (`Open PR`→`open_pr`, `Merge`→`merge`, `Review`→`review`, `Implement`→`implement`, …) and otherwise to a plain `implement` judgment kind with no special powers. `sb plan edit` carries the kind for any Step it adds, using the same `Name:kind` form, so a Plan can grow a real `open_pr`/`merge` Step later — kind is never inferred from the display name after creation. Further high-level operations may be added if they represent genuine recurring lifecycle actions. Underlying database operations may remain granular internally; agents interact with the higher-level transaction. Editing semantics — whole documents, agent-writable fields only, serialization handled by Switchboard — are defined in Section 3.
...
* an unresolved `major` review Finding blocking a merge (Section 4)
...
* `active` or `failed` Steps whose owner has stopped and whose wake did not land or was not acted on within `owner_wake_timeout`
* Steps whose bundled operation failed and was not recovered
* a Task with an open coordination need and no eligible coordinator (Section 1)
...
* a scope-narrowing confirm item — an agent removed a `scope.in`/constraint/success-criterion (Section 3)
...
New Task, Auto Task, open/focus an agent terminal, answer a Question, approve a PR (as a head-bound `approval` or a `pre-approval`), confirm or flag a relayed authorization, confirm or revert a scope narrowing, optionally merge, request changes (reopen), retry a failed Step, assign a Task coordinator, close a Task, restore the environment, refresh the Advisor, edit repo configuration, and spawning an agent as a convenience.
...
Logged events include: Task creation and close; Plan creation, change and completion; Step assignment, completion, release, reopen and steal; review Findings raised and resolved; merge authorizations granted, confirmed, invalidated and revoked; scope-change records; coordination needs raised and resolved; agent spawn, done and cleanup; parent/child relationships; handoffs; `tell`, `ask` and `done` messages; question answers, escalations and withdrawals; review activity; worktree creation and cleanup; PR creation and merge; role/model/preset used; important status transitions; timestamps and durations.
...
`advisor` asks create no Question (synchronous). A human-targeted Question the agent left unresolved after a **user-initiated** turn is marked `possibly answered out of band` and drops out of `Needs You` (still unresolved for `Ready to Close`, still shown in the Task view); it **returns to `Needs You` after `orphan_question_timeout`** if still unresolved, so it cannot vanish permanently on a coincidence. An orphaned Question — asker not restorable, or `completed`-but-live and unprocessed past the timeout — is auto-`withdrawn` or auto-`resolved` respectively (Section 7).
...
| `active` | eligible (all preceding Steps complete), incomplete, not blocked or failed | completed; or an earlier Step is `reopen`ed → back to `pending` | derived | **yes** if `unowned`; or if its owner has stopped and is not restorable, or the wake was delivered but not acted on within `owner_wake_timeout` |
| `blocked` | eligible, awaiting a tracked merge precondition — approval, CI not `red`, or an unresolved `major` Finding | last precondition met → `active` (owner merges); merge observed → `complete`; earlier Step reopened → `pending` | derived | no |
| `failed` | a bundled sub-operation or local check failed; or a system Step whose remote fact was later observed false (PR closed) | `step retry` succeeds → `complete`; retry fails → stays `failed` | Switchboard | **yes** if its owner has stopped and is not restorable, or the wake was delivered but not acted on within `owner_wake_timeout`; else if unrecovered past `failed_step_timeout`; a `failed` owned Step also blocks the owner's `sb done` |
...
A Step also carries an immutable **kind** (`implement`, `review`, `open_pr`, `merge`, …), set at creation, which — not the display name — decides system-vs-judgment completion and review-independence; a Plan with commits cannot reach `complete` without a completed `merge`-kind Step (Section 4). A review produces durable **Findings** (`major`/`minor`/`nit`); an unresolved `major` blocks both `Review` completion and `Merge`, and becomes an attention item past `failed_step_timeout` (Section 4).
...
| `awaiting merge` | `Open PR` complete, a live authorization, `Merge` incomplete | `Merge` complete → `complete`; authorization invalidated (push/revoke) → `Needs Human Review`; earlier Step reopened → `in progress` | derived | no (the owner drives it; a red-CI or open-major item covers any block) |
...
| `Stalled` | unfinished work and no agent advancing it: a `stalled` agent, or every agent idle with no explanation, or no agent `live` and none `restorable` | the condition resolves | derived | **yes** |
| `Ready to Close` | the single Section 4 predicate: work has happened (a completed Plan, or a Plan-less Task's initiating agent has `sb done`), every Plan complete, no open Step/Question, no agent working | more work starts, or closed | derived | no |
