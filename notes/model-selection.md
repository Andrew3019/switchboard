...
| **builder** — the code-writing leaf, asked for by name | GPT-5.6-sol, medium | GPT over Claude for code | GPT over Claude for code | **Opus 5, medium** (`opus-5-medium`) — restored as the role's default after a brief Luna detour. The direct-path Luna tier remains available by explicit per-spawn selection when the work's requirements are already settled. |
