...
| **builder** — the code-writing leaf, asked for by name | GPT-5.6-sol, medium | GPT over Claude for code | GPT over Claude for code | **GPT-5.6-Luna, max** (`gpt-luna-max-effort`) since 2026-09-07 — back on codex after six days on `opus-5-medium`. Cheap good code at maximum effort is this role's ORDINARY case, so it became the role's default rather than a flag every caller had to remember. The role is the only shipped one on a gated tier, which is why `[routing] gpt_luna_direct_enabled` now ships true |
...
| **direct-path implementation** (a tier, not a role) | n/a | n/a | n/a | **GPT-5.6-Luna, max** (`gpt-luna-max-effort`) — the cheap model thinking as hard as it can, for work whose requirements are already settled. `builder`'s default; still nameable per spawn on a `worker`, which is deliberately kept open. `lead`, `dispatcher` and `reviewer` are refused it in `defaults/models.toml`. Ships ON since 2026-09-07, and carries a raised context budget (872k window, 650k auto-compact) |
