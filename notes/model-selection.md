...
| **builder** — the code-writing leaf, asked for by name | GPT-5.6-sol, medium | GPT over Claude for code | GPT over Claude for code | **Opus 5, medium** (`opus-5-medium`) again since 2026-09-10 — it was this tier from 2026-09-01, spent three days on `gpt-luna-max-effort` as the one shipped role on codex, and is back (Andrew). A role file cannot know whether the next job handed to a builder is direct-path, so the cheap tier goes back to being chosen per spawn by whoever does |
