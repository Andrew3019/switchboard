...
| **builder** — the code-writing leaf, asked for by name | GPT-5.6-sol, medium | GPT over Claude for code | GPT over Claude for code | **Opus 5, medium** (`opus-5-medium`) since 2026-09-01 — the codex pin is RETIRED, not switchable. What it was for (good code cheaply, on already-clear requirements) is now `gpt-luna-max-effort`, chosen per spawn by whoever knows the work is direct-path rather than by a role file deciding for every builder ever spawned |
...
| **direct-path implementation** (a tier, not a role) | n/a | n/a | n/a | **GPT-5.6-Luna, max** (`gpt-luna-max-effort`) — the cheap model thinking as hard as it can, for work whose requirements are already settled. Per-spawn on `worker`/`builder` only; `lead`, `dispatcher` and `reviewer` are refused it in `defaults/models.toml`, and the whole tier is off until `[routing] gpt_luna_direct_enabled` is true |
