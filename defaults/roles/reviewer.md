...
NO SHIPPED ROLE IN `defaults/roles/` IS ON `strong` — the one exception is the plans
plugin's `planner`, and it is spawned once per shaped task rather than fanned out.
designer.md was on it too, and its argument was sound as far as it went — design is one
of the places a better model actually pays, and it was rare enough that the cost was
bounded. What did not follow was pinning the tier to a ROLE.
